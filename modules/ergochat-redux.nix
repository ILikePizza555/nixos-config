    ...
    services.ergochat-redux = {
      ...
      opers = mkOption {
        type = types.attrsOf (mkSubmoduleFromOptions {
          ...
          certfp = mkSimpleOption (types.nullOr types.str) null "if a SHA-256 certificate fingerprint is configured here, then it will be required to /OPER ";
          auto = mkSimpleOption types.bool false "if 'auto' is set (and no password hash is set), operator permissions will be granted automatically as soon as you connect with the right fingerprint";
        });
        default = {};
      };
